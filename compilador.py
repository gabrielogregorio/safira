...
global txTelaLogs  # Text de logs
...
def sintaxe_linha(palavra,cor,frase,linha):
    conteudo = frase
    if palavra in frase:
        txTelaProgramacao.tag_add(palavra, linha+str('.')+str(conteudo.find(palavra)), linha+str('.')+str(conteudo.find(palavra)+len(palavra)))
        txTelaProgramacao.tag_config(palavra, fg=cor)
...
menu_arquivo.add_command(label='Salvar Como (Ctrl-Shift-S)',command=dialog_salvar_arquivo)
menu_arquivo.add_separator()
